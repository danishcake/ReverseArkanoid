...
public:
	...
	Widget(NinePatch _tiles, int _width, int _height);
	virtual ~Widget(void);
	void Delete();
	...
	static MouseEvent OnGlobalMouseUp;
	...
	/* Mouse cursor */
	static void SetMouseCursorEnabled(bool _enabled){cursor_enabled_ = _enabled;}
	static void SetMouseCursor(BlittableRect* _rect);

	/* Screen size */
	static void SetScreenSize(Vector2i _screen_size){screen_size_ = _screen_size;}
	static Vector2i GetScreenSize(){return screen_size_;}
	static Vector2i GetScreenCentre(){return screen_size_ / 2;}
};
